...
export default defineEventHandler(async (event) => {
	...
	try {
		...
		const authenticator = user.devices.find(
			(device) => device.credentialID === body.id
		);
		if (!authenticator) {
		...
		let verification = await verifyAuthenticationResponse({
			response: body,
			expectedChallenge,
			expectedOrigin: `${useRuntimeConfig().public.rpProtocol}://${
				useRuntimeConfig().public.rpId
			}:3000`,
		...
		await userCollection.findOneAndUpdate(
			{ _id: user._id, "devices._id": authenticator._id },
			{ $set: { "devices.$.counter": newCounter } }
		);
		return {
			verified,
			status: "success",
